...
function inlineCss() {
  return {
    ...
    writeBundle(outputOptions: { dir?: string }) {
      const outDir = outputOptions.dir || resolve(import.meta.dirname, "../dist/plo-dw01");
      ...
      while ((match = cssLinkRegex.exec(html)) !== null) {
        ...
        const cssFile = cssHref.replace(/^\/p\/plo-dw01\//, "");
...
export default defineConfig({
  ...
  build: {
    outDir: path.resolve(import.meta.dirname, "../dist/plo-dw01"),
    emptyOutDir: true,
    rollupOptions: {
      output: {
        manualChunks: {
