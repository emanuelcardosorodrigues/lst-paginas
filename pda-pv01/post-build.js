...
const distDir = path.resolve(__dirname, "../dist/pda-pv01");
...
// Remove blocking stylesheet links injected by Vite for these CSS files
for (const cssFile of cssFiles) {
  const blockingPattern = new RegExp(
    `\\s*<link rel="stylesheet" crossorigin href="/p/pda-pv01/assets/${cssFile.replace(".", "\\.")}">`,
    "g"
  );
...
const cssLinks = cssFiles
  .map((cssFile) => {
    const cssPath = `/p/pda-pv01/assets/${cssFile}`;
