...
function inlineCss() {
  return {
    ...
    writeBundle(outputOptions: { dir?: string }) {
      const outDir = outputOptions.dir || resolve(import.meta.dirname, "../dist/plo-up01");
      ...
      while ((match = cssLinkRegex.exec(html)) !== null) {
        ...
        const cssFile = cssHref.replace(/^\/p\/plo-up01\//, "");
...
export default defineConfig({
  ...
  build: {
    outDir: path.resolve(import.meta.dirname, "../dist/plo-up01"),
    emptyOutDir: true,
    rollupOptions: {
      output: {
        manualChunks: {
